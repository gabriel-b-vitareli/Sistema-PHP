<header>
    <nav>
        <a href="/sistema-php/index.php">Início</a> | 
        <a href="/sistema-php/app/create.php">Cadastrar</a> | 
        <a href="/sistema-php/app/delete.php">Excluir</a> | 
        <a href="/sistema-php/app/select.php">Relatório</a> | 
        <a href="/sistema-php/app/select_w.php">Aluno</a> | 
        <a href="/sistema-php/app/update.php">Atualizar</a> | 
        <a href="/sistema-php/login/login.php">Login</a> | 
        <a href="/sistema-php/login/logout.php">Sair</a>
    </nav>
</header>
<hr>